...
var chalkGetter = require('../chalk-getter');
...
ComplexBeautifier.prototype.beautify = function(err) {
  ...
  var exerpt = err.getExerpt(this.options);

  return (err.ns && chalk.grey('[') + chalk.magenta(err.ns.split(':').join(chalk.grey(':'))) + chalk.grey('] ')) +
    err.name + chalk.grey(': ') + chalk.red.bold(err.message || '') +

    chalk.grey(' − code ' + chalk.yellow(err.code)) + '\n' +
    ...
    _.reduce(err.parsedStack, function(acc, ctx, pos) {
      ...
      return acc +
        ...
        chalk.grey(ctx.prefixDirname) + ctx.shortDirname + fileColor(ctx.basename) +
        // add line & column
