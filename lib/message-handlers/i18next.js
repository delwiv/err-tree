...
module.exports = function(options) {
  if (!options)
    throw new makeError.ErrtreeError.assert(options, 'The options parameter is required to activate the i18next message handler');
  else if (!_.isObject(options))
    throw new makeError.ErrtreeError('Invalid i18next options parameter: it should be an object');
  else if (!options.i18next || !options.i18next.init)
    throw new makeError.ErrtreeError('Invalid i18next options.i18next: please provide a valid instance of i18next');
